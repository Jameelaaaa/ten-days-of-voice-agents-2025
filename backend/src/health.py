...
class Assistant(Agent):
    ...
    async def finalize_checkin(self, context: RunContext):
        ...
        # Load existing data from file to ensure we have the latest
        self.load_past_checkins()
        
        # Handle multiple check-ins per day by creating an array
        if today in self.past_checkins:
            # If it's already a list, append to it
            if isinstance(self.past_checkins[today], list):
                self.past_checkins[today].append(final_checkin)
            else:
                # Convert single entry to list and add new entry
                existing_entry = self.past_checkins[today]
                self.past_checkins[today] = [existing_entry, final_checkin]
        else:
            # First check-in for this date
        ...
        try:
            ...
            for entries in self.past_checkins.values():
                if isinstance(entries, list):
                    total_interactions += len(entries)
                else:
                    total_interactions += 1
            
            logger.info(f"Check-in saved to {self.wellness_log_file} (total interactions: {total_interactions})")
            ...
            return f"Check-in finalized! {final_checkin['summary']}. Thank you for sharing!"
    ...
    def get_past_context(self):
        ...
        latest_entries = self.past_checkins[latest_date]
        
        # If there are multiple entries for the latest date, get the most recent one
        if isinstance(latest_entries, list):
            # Sort by timestamp and get the latest
            latest_entries.sort(key=lambda x: x.get('timestamp', ''), reverse=True)
            latest_checkin = latest_entries[0]
            interaction_count = len(latest_entries)
            interaction_note = f" (we've had {interaction_count} check-ins today)"
        else:
            latest_checkin = latest_entries
            interaction_note = ""
        
        context_parts = []
        context_parts.append(f"Last time we talked on {latest_date}{interaction_note}, you mentioned:")
    ...
    async def get_past_checkins_info(self, context: RunContext, days_back: int = 7):
        ...
        for date_str, entries in self.past_checkins.items():
            try:
                ...
                if days_diff <= days_back:
                    ...
                    if isinstance(entries, list):
                        for i, checkin in enumerate(entries):
                            recent_checkins.append((days_diff, checkin, f"#{i+1}"))
                    else:
                        recent_checkins.append((days_diff, entries, ""))
            except ValueError:
                continue
        ...
        recent_checkins.sort(key=lambda x: (x[0], x[2]))
        
        summary_parts = [f"Recent check-ins (last {days_back} days):"]
        for days_ago, checkin, entry_num in recent_checkins:
            if days_ago == 0:
                day_desc = "today"
            elif days_ago == 1:
                day_desc = "yesterday"
            ...
            entry_label = f"{day_desc.title()}{entry_num}"
            summary_parts.append(f"- {entry_label}: {checkin['mood']} mood, {checkin['energy']} energy")
